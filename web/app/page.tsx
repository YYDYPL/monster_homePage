    ...
    <>
        ...
        <div className="container hero-grid">
          ...
          {config.heroImageUrl ? (
            <div className="hero-image-card hero-reveal hero-reveal-delay">
              <img className="hero-main-image" src={config.heroImageUrl} alt={`${config.ownerName || config.siteName} ??????`} />
              <div className="hero-image-overlay" aria-hidden="true" />
              <div className="hero-image-caption">
                {config.avatarUrl ? (
                  <img className="hero-avatar" src={config.avatarUrl} alt={`${config.ownerName} ???`} />
                ) : (
            ...
            <div className="terminal-card hero-reveal hero-reveal-delay" aria-label="?????????">
      ...
      <section className="stats-strip">
        <div className="container stats-grid">
          <div className="stat-item"><strong>{postResponse.success ? postResponse.data.total : posts.length}+</strong><span>????</span></div>
          <div className="stat-item"><strong>{noteResponse.success ? noteResponse.data.total : notes.length}+</strong><span>????</span></div>
          <div className="stat-item"><strong>{projectResponse.success ? projectResponse.data.total : projects.length}+</strong><span>????</span></div>
          <div className="stat-item"><strong>?</strong><span>????</span></div>
        </div>
      </section>

      <section className="section motion-section"><div className="container"><SectionHeading eyebrow="Latest writing" title="????" href="/blog" /><div className="content-grid">{posts.slice(0, 4).map((post, index) => <PostCard post={post} featured={index === 0 && posts.length > 2} key={post.id} />)}</div></div></section>
      <section className="section alt motion-section"><div className="container"><SectionHeading eyebrow="Knowledge base" title="????" href="/notes" /><div className="note-list">{notes.slice(0, 4).map((note) => <NoteCard note={note} key={note.id} />)}</div></div></section>
      <section className="section motion-section"><div className="container"><SectionHeading eyebrow="Selected work" title="????" href="/projects" /><div className="content-grid three">{projects.slice(0, 3).map((project) => <ProjectCard project={project} key={project.id} />)}</div></div></section>
      <section className="section alt connect-section motion-section"><div className="container"><div className="section-heading"><div><p className="eyebrow">Let&apos;s connect</p><h2>?????????</h2></div><Link className="button primary" href="/contact">??? <ArrowIcon /></Link></div></div></section>
    </>
  );
}

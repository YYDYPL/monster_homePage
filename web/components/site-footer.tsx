...
const groups = [
  { title: "??", links: [["????", "/blog"], ["????", "/notes"], ["????", "/projects"], ["????", "/lab"]] },
  { title: "??", links: [["???", "/about"], ["????", "/resume"], ["????", "/uses"], ["????", "/links"]] },
  { title: "??", links: [["????", "/search"], ["???", "/contact"], ["RSS ??", "/rss.xml"], ["????", "/admin"]] },
];
      ...
      <div className="container footer-grid">
        <div className="footer-intro">
          ...
          <div className="social-row">
            ...
            {config.xiaohongshuUrl && <Social href={config.xiaohongshuUrl} label="???">RED</Social>}
            {config.douyinUrl && <Social href={config.douyinUrl} label="??">DY</Social>}
            {config.wechatQrCodeUrl ? <Social href={config.wechatQrCodeUrl} label="??">WX</Social> : config.wechat ? <Link href="/contact" title={`???${config.wechat}`} aria-label="??">WX</Link> : null}

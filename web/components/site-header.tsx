  ...
  return (
    <header className="site-header">
      <div className="container header-inner">
        ...
        <nav className={`main-nav ${open ? "is-open" : ""}`} aria-label="???">
          {navigation.map((item) => {
        ...
        <div className="header-actions">
          <Link className="icon-button search-button" href="/search" aria-label="??"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg></Link>
          <button className="icon-button" type="button" onClick={toggleTheme} aria-label="????"><ThemeIcon theme="light" /></button>
          <button className="menu-button" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label="????"><span /><span /><span /></button>
        </div>
      </div>
    </header>
  );
